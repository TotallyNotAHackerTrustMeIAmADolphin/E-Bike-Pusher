  ...
  void onWrite(BLECharacteristic *pCharacteristic)
  {
    ...
    if (rxValue.length() > 0)
    {
      ...
      else if (rxValue.startsWith("SET:"))
      {
        ...
        if (key == "th")
          deviceInfo.brakingThreshold = val.toInt();
        if (key == "ti")
          deviceInfo.brakingTimeout = val.toInt();
        if (key == "tm")
          deviceInfo.torqueMultiplier = val.toFloat();
        if (key == "ba")
          deviceInfo.brakeAlpha = val.toFloat();
...
void loop()
{
  ...
  if (millis() - last_cmd_time >= 20)
  {
    ...
    odrive.requestData(CMD_GET_VBUS_VOLTAGE); // NEW: Request Voltage & Battery Amps!
  }
  ...
  if (deviceConnected && millis() - last_dashboard_time >= 500)
  {
    ...
    // TRUE Electrical Power = Battery Voltage * Battery Current
    float elec_power = odrive.getVoltage() * odrive.getBusCurrent();

    char json[150];
    snprintf(json, sizeof(json), "{\"c\":%d,\"p\":%.1f,\"v\":%.1f,\"a\":%.1f,\"pr\":%.2f,\"b\":%d,\"th\":%d,\"ti\":%d,\"tm\":%.3f,\"ba\":%.2f}",
