...
void runMaintenanceMode()
{
  ...
  {
    char m[64];
    snprintf(m, sizeof(m), "Connected! IP: %s", WiFi.localIP().toString().c_str());
    addLog(m);
  }
  else
  {
...
void loop()
{
  ...
  if (millis() - last_cmd_time >= 20)
  {
    ...
    if (brake_avg > 0.0f)
    {
      // 1. PULLING: Push the bike, but ONLY if pedaling!
    ...
    {
      // 2. BRAKING: Trailer is pushing the bike.
      // THE FIX: ONLY apply regen braking if the wheel is actually rolling forward!
      // (0.05 rev/s is a tiny deadband that prevents reversing at a standstill)
      ...
      {
        target_motor_torque = 0.0f; // Stop braking so we don't go backwards!
      }
    }

    // Send the command and poll for the next cycle's data
