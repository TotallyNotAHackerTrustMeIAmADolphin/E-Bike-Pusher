            ...
            if (rxValue == "OTA") triggerOTA();
                ...
                if(s1 > 0 && s2 > 0) {
                    ...
                    if (new_tm != deviceInfo.torqueMultiplier) {
                        char m[64]; snprintf(m, sizeof(m), "Tuning: Max Force %.2fA -> %.2fA", deviceInfo.torqueMultiplier, new_tm); addLog(m);
                        deviceInfo.torqueMultiplier = new_tm;
                    }
                    if (new_tc != deviceInfo.brakeTimeConstant) {
                        char m[64]; snprintf(m, sizeof(m), "Tuning: Filter Time %.2fs -> %.2fs", deviceInfo.brakeTimeConstant, new_tc); addLog(m);
